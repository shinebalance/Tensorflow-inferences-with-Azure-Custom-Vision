...
# from jpg file to tensor
def convertVideo2tensor(stream, network_input_size, isShapePeep):
    image = resize_down_to_1600_max_dim(stream)
    if isShapePeep is True:
        print('to 1600 >>>', image.shape)
    # We next get the largest center square
    h, w = image.shape[:2]
...
def main():
    ...
    try:
        ...
        while(True):
            ...
            image_f32 = convertVideo2tensor(
                frame, network_input_size=NETWORK_INPUT_SIZE, isShapePeep=False
                )
            ...
            caption = 'Classified as >>>{}  Score :{}'.format(labels[result], score)
            frame = cv2.putText(
